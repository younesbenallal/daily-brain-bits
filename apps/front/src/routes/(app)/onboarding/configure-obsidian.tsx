...
function ConfigureObsidianPage() {
	...
	const statusQueryOptions = useMemo(
		() => orpc.obsidian.status.queryOptions(),
		[],
	);
	...
	const generateKeyMutation = useMutation({
		mutationFn: async () => {
			...
			const obsidianKeys = existing.filter(
				(key) =>
					key.prefix === "dbb_obsidian" || key.name === "Obsidian Plugin",
			);
			...
			if (obsidianKeys.length > 0) {
				await Promise.all(
					obsidianKeys.map(async (key) => {
						...
						if (result?.error) {
							throw new Error(
								result.error.message || "Failed to rotate API key.",
							);
						}
					}),
				);
			...
			if (createResult?.error || !createResult?.data?.key) {
				throw new Error(
					createResult?.error?.message || "Failed to generate API key.",
				);
			}
	...
	const handleSaveGlob = () => {
		setGlobMutation.mutate({
			glob: globValue.trim().length > 0 ? globValue.trim() : null,
		});
	};
	...
	const vaultLabel =
		...
		(statusData?.vaultId
			? `Vault ${statusData.vaultId.slice(0, 8)}`
			: "Obsidian Vault");
	const statusLabel = connected
		? `Connected to ${vaultLabel}`
			...
			<div className="space-y-6">
				<div className="space-y-3">
					<div className="flex items-center gap-3">
						...
						<h1 className="font-display text-3xl text-[#2d71c4]">
							Configure Obsidian
						</h1>
					</div>
					<p className="text-sm text-[#737373]">
						Tell us what notes you would like to receive in your inbox
					</p>
				</div>

				<div className="space-y-3">
					<p className="font-ui text-base font-semibold text-[#163c6b]">
						Install the plugin
					</p>
					<p className="text-sm text-[#737373]">
						Open the GitHub repo to follow the plugin install steps.
					</p>
					<Button
						type="button"
						variant="outline"
						...
						onClick={() =>
							window.open(pluginInstallUrl, "_blank", "noopener,noreferrer")
						}
					>
				...
				<div className="space-y-3">
					...
					<p className="text-sm text-[#737373]">
						Generate a token, then paste it into the plugin settings inside
						Obsidian.
					</p>
					...
					{apiKeyError ? (
						<p className="text-sm text-[#ef4444]">{apiKeyError}</p>
					) : null}
				</div>
				...
				<div className="space-y-3">
					...
					{!connected ? (
						<p className="text-xs text-[#737373]">
							Add the token in Obsidian to finish linking. This page will update
							automatically.
						</p>
					) : null}
				</div>
				...
				<div className="space-y-3">
					...
					<p className="text-sm text-[#737373]">
						Pattern of the path of the notes you want to receive reminders for
					</p>
					<div className="flex flex-col gap-2">
						<Input
						...
						<div className="flex items-center gap-3">
							...
							>
								{setGlobMutation.isPending ? (
									<Loader2Icon className="h-4 w-4 animate-spin" />
								) : null}
								Save filter
							</Button>
							{setGlobMutation.isSuccess ? (
								<span className="text-xs text-[#16a34a]">Saved</span>
							) : null}
						</div>
						{setGlobMutation.isError ? (
							<p className="text-sm text-[#ef4444]">Failed to save filter.</p>
						) : null}
						{!connected ? (
							<p className="text-xs text-[#737373]">
								Connect the plugin first to enable filtering.
							</p>
						) : null}
					</div>
