...
export const TagsTrigger = ({ className, children, placeholder = "Select a tag...", ...props }: TagsTriggerProps) => (
	<PopoverTrigger
		render={<Button className={cn("h-auto min-h-10 w-full justify-start p-2", className)} role="combobox" variant="outline" {...(props as any)} />}
	>
		<div className="flex flex-wrap items-center gap-1">
			{children}
			{(!children || (Array.isArray(children) && children.length === 0)) && <span className="px-2 py-px text-muted-foreground">{placeholder}</span>}
		</div>
	</PopoverTrigger>
);
...
export type TagsContentProps = React.ComponentPropsWithoutRef<typeof PopoverContent> & {
	commandProps?: React.ComponentPropsWithoutRef<typeof Command>;
};

export const TagsContent = ({ className, children, commandProps, ...props }: TagsContentProps) => {
...
export const TagsInput = ({ className, ...props }: TagsInputProps) => <CommandInput className={className} {...(props as any)} />;

export type TagsListProps = ComponentProps<typeof CommandList>;

export const TagsList = ({ className, ...props }: TagsListProps) => <CommandList className={cn("max-h-[200px]", className)} {...(props as any)} />;
