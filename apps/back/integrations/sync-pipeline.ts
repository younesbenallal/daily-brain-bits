...
async function filterObsidianItems(connectionId: number, items: unknown[], defaultDeletedAtSource: string): Promise<ScopeFilterResult> {
	...
	for (const item of items) {
		...
		const path =
			itemRecord && "metadata" in itemRecord && typeof (itemRecord as { metadata?: { path?: unknown } }).metadata?.path === "string"
		...
		filteredItems.push({
			...safeRecord,
			deletedAtSource: typeof safeRecord.deletedAtSource === "string" ? (safeRecord.deletedAtSource as string) : defaultDeletedAtSource,
			metadata: {
				...((itemRecord?.metadata && typeof itemRecord.metadata === "object" && !Array.isArray(itemRecord.metadata)
					? itemRecord.metadata
					: {}) as Record<string, unknown>),
