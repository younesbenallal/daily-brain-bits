import { db, integrationConnections, integrationScopeItems } from "@daily-brain-bits/db";
...
function parseSecrets(raw: string | null): NotionSecrets | null {
  if (!raw) {
    return null;
  }
  try {
    const parsed = JSON.parse(raw) as NotionSecrets;
...
async function getActiveNotionConnection(userId: string) {
  const connections = await db
    .select({
      ...
      secretsJsonEncrypted: integrationConnections.secretsJsonEncrypted,
      updatedAt: integrationConnections.updatedAt,
    })
    .from(integrationConnections)
    .where(and(eq(integrationConnections.userId, userId), eq(integrationConnections.kind, "notion")))
  ...
  const secrets = parseSecrets(connection.secretsJsonEncrypted);
  if (!secrets) {
    return null;
  }
...
const status = baseRoute
  ...
  .handler(async ({ context }) => {
    ...
    const config =
      connectionBundle.connection.configJson &&
      typeof connectionBundle.connection.configJson === "object" &&
      !Array.isArray(connectionBundle.connection.configJson)
        ? (connectionBundle.connection.configJson as { workspaceName?: string | null; workspaceIcon?: string | null })
        : {};
...
const searchDatabases = baseRoute
  ...
  .handler(async ({ context, input }) => {
    ...
    const results = await searchNotionDatabases(connectionBundle.secrets.accessToken, input.query);
