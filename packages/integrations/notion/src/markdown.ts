...
function renderListItem(marker: string, block: NotionBlockWithChildren, depth: number): string {
...
function appendChildren(base: string, block: NotionBlockWithChildren, depth: number): string {
...
function blocksToMarkdownWithDepth(blocks: NotionBlockWithChildren[], depth: number): string {
	let markdown = "";
	let prevWasList = false;
...
function applyAnnotations(text: string, annotations: RichTextItemResponse["annotations"]): string {
	let result = text;
