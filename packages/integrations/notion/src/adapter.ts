...
import type { NotionRequest, NotionSyncCursor, NotionSyncOptions } from "./types";

export type NotionSyncScope = {
	databaseId: string;
};

export type NotionAdapterOptions = Omit<NotionSyncOptions, "cursor" | "request"> & {
...
export function createNotionSyncAdapter(
	...
	const { notion, request: providedRequest, pageSize, safetyMarginSeconds, maxPages, onPage } = options;
